...
import (
	...
	"com.example.bookingmanagement/eurekaregistry/helper"
...
func (erm EurekaRegistrationManager) RegisterWithSerivceRegistry(eurekaConfigs RegistrationVariables) {
	...
	helper.MakePostCall(eurekaConfigs.ServiceRegistryURL+"bookingmanagementmodule", body, nil)
	time.Sleep(10 * time.Second)
	bodyUP := erm.getBodyForEureka("UP", eurekaConfigs)
	helper.MakePostCall(eurekaConfigs.ServiceRegistryURL+"bookingmanagementmodule", bodyUP, nil)
}

func (erm EurekaRegistrationManager) SendHeartBeat(eurekaConfigs RegistrationVariables) {
	logger.Infof("Sending HeartBeat")
	job := func() {
		helper.MakePutCall(eurekaConfigs.ServiceRegistryURL+"bookingmanagementmodule/"+eurekaConfigs.InstanceId, nil, nil)
	}
...
func (erm EurekaRegistrationManager) DeRegisterFromServiceRegistry(configs RegistrationVariables) {
	...
	helper.MakePostCall(configs.ServiceRegistryURL+"bookingmanagementmodule", bodyDOWN, nil)
}
...
func (erm EurekaRegistrationManager) getBodyForEureka(status string, configs RegistrationVariables) *AppRegistrationBody {
	...
	applicationName := "bookingmanagementmodule"
	env := os.Getenv("GO_MICRO_ACTIVE_PROFILE")
	if env == "prod" {
		hostname = "bookingmanagementmodule"
	}
